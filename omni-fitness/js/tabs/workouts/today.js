...
function renderActive(container, session) {
    ...
    <div class="wk-today-bar" id="wkSessionBar">
        ...
        <div class="wk-today-bar-actions">
            <button class="wk-today-btn danger sm" onclick="wkCancelSession()" title="Cancel">
                ${window.Icons.close(14)}
                <span class="wk-hide-mobile">Cancel</span>
            </button>
            <button class="wk-today-btn green sm" onclick="wkOpenFinishSummary()" title="Finish">
                ${window.Icons.check(14)}
                <span style="margin-left: 4px;">Finish</span>
            </button>
        </div>
    </div>
    ...
    <div class="wkt-hud" id="wktHud">
        ...
        <div class="wkt-hud-body">
            ...
            <div class="wkt-ring-col">
                <div class="wkt-ring-wrap" id="wktRingWrapOuter">
                    <svg class="wkt-svg" viewBox="0 0 320 320" aria-hidden="true" id="wktSvg">
                        <defs>
                            <mask id="outerDimMask">
                                <circle cx="160" cy="160" r="144" stroke="white" stroke-width="16" fill="transparent" />
                            </mask>
                        </defs>
                        
                        <!-- Dimmed static track (to show grey for unmodified) -->
                        <circle class="wkt-ring-track outer-track" cx="160" cy="160" r="144"
                            stroke-dasharray="904.8 904.8" stroke-dashoffset="0"/>

                        <!-- Colored ring track segments (dimmed) -->
                        <g id="wktOuterSegmentsTrack" mask="url(#outerDimMask)" style="opacity: 0.2;"></g>

                        <!-- Colored ring progress segments (masked by progress) -->
                        <g id="wktOuterSegments"></g>
                ...
                <div class="wkt-controls">
                    <button class="wkt-btn-ctrl" id="wktBtnPrev"
                        title="Previous" onclick="window.wkRunner && window.wkRunner.prev()">
                        <svg width="20" height="20" viewBox="0 0 24 24" fill="currentColor"><path d="M6 6h2v12H6zm3.5 6l8.5 6V6z"/></svg>
                    </button>
                    <button class="wkt-btn-ctrl primary" id="wktBtnPlay"
                        onclick="window.wkRunner && window.wkRunner.togglePlay()">
                        <svg width="24" height="24" viewBox="0 0 24 24" fill="currentColor"><path d="M8 5v14l11-7z"/></svg>
                    </button>
                    <button class="wkt-btn-ctrl" id="wktBtnNext"
                        title="Next" onclick="window.wkRunner && window.wkRunner.next()">
    ...
    <div class="wkt-logger-divider">
        <span style="font-size: 16px; color: var(--text);">Workout Details</span>
    </div>
...
function renderExercise(ex) {
    ...
    const progressRaw = ex.progressionRule && ex.progressionRule !== 'none' ? ex.progressionRule : null;
    const progress = progressRaw ? (RT_PROG_RULES[progressRaw] || progressRaw) : null;
    ...
    <div class="wk-today-ex ${allDone ? 'complete' : ''}" data-ex-id="${esc(ex.id)}">
        <div class="wk-today-ex-head">
            <div style="min-width:0;">
                <p class="wk-today-ex-name"><span style="display:inline-block; width:8px; height:8px; border-radius:50%; background-color:${ex.color || window.EXERCISE_PALETTE[0] || '#FF3B30'}; margin-right:6px; margin-bottom:1px;"></span>${esc(ex.exerciseName || ex.exerciseNameSnapshot || ex.exerciseId)}</p>
...
function renderSetRow(sexId, idx, set, tt, units, isCurrent, stepId) {
    ...
    if (tt === 'time') {
        ...
        cells += cell(done ? fmtW(set.weight) : '');
        cells += cell(done ? fmtDur(set.durationSec) : '');
    } else if (tt === 'distance_time') {
        cells += cell(done ? `${fmtW(set.distance)} ${esc(set.distanceUnit || 'km')}` : '');
        cells += cell(done ? fmtDur(set.durationSec) : '');
    } else if (tt === 'bodyweight_reps') {
        cells += cell(done ? fmtR(set.reps) : '');
        cells += cell(done ? fmtW(set.addedWeight) : '');
    ...
    cells += `<div class="wk-today-set-cell" onclick="event.stopPropagation(); window.wkToggleSetLog('${esc(sexId)}', ${idx})">${done ? '<span class="wk-today-set-checkmark">✓</span>' : '<span class="wk-today-set-empty">tap</span>'}</div>`;
...
function startElapsedTicker() {
    ...
    _elapsedInterval = setInterval(() => {
        const session = window.wkState && window.wkState.activeSession;
        if (!session) { clearInterval(_elapsedInterval); _elapsedInterval = null; return; }
        const formatted = window.wkTodayHelpers.fmtElapsed(session.startedAt);
        const node1 = document.getElementById('wkElapsedTime');
        if (node1) node1.textContent = formatted;
        const node2 = document.getElementById('wktTsElapsed');
        if (node2) node2.textContent = formatted;
        const node3 = document.getElementById('wktStatElapsed');
        if (node3) node3.textContent = formatted;
    }, 1000);
}
