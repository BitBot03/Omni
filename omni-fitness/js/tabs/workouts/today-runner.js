            ...
            if (!isGroup) {
                ...
                for (const ex of exs) {
                    ...
                    for (let i = 1; i <= totalSets; i++) {
                        ...
                        if (i < totalSets) {
                            ...
                            steps.push({ type:'REST', durationSec: rest, label: `Rest · ${ex.exerciseName || 'Next set'}` });
                        }
                    }
                }
            } else {
                ...
                for (let round = 1; round <= maxRounds; round++) {
                    ...
                    if (round < maxRounds && blockRest > 0) {
                        steps.push({ type:'REST', durationSec: blockRest, label: `${block.name||'Round'} · round ${round} rest` });
                    }
                }
            }
        };
    ...
    _renderQueue() {
        ...
        el.innerHTML = slice.map((step, i) => {
            ...
            return `<div class="wkt-queue-item ${isCurrent ? 'current' : ''} ${isPast ? 'past' : ''} ${isRest ? 'rest' : ''}">
                <span class="wkt-qi-dot"></span>
                <span class="wkt-qi-text">${esc(name)}</span>
            </div>`;
        }).join('');
    }
    ...
    _updateMiniHud(label, time, isRest, innerFrac, outerFrac, playIcon) {
        ...
        mini.innerHTML = `
            <div class="wkt-mini-ring-wrap">
                ...
                <span class="wkt-mini-pct">${Math.round(outerFrac*100)}%</span>
            </div>
...
/* ── Bottom sheet toggle ─────────────────────────────────── */
window.wktToggleSheet = function() {
    const sheet = document.getElementById('wktSheet');
    const backdrop = document.getElementById('wktSheetBackdrop');
    if (!sheet) return;
    const isOpen = sheet.classList.contains('open');
    if (!isOpen) {
        // Populate sheet body with current logger content
        const logger = document.getElementById('wkTodayPlayer');
        const body   = document.getElementById('wktSheetBody');
        if (logger && body) body.innerHTML = logger.innerHTML;
    }
    sheet.classList.toggle('open', !isOpen);
    backdrop && backdrop.classList.toggle('visible', !isOpen);
    document.body.classList.toggle('wkt-sheet-open', !isOpen);
};
