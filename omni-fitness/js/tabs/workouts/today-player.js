...
/* ─────────────────────────────────────────────────────────────
   AUTO LOG SET (SILENT)
   ───────────────────────────────────────────────────────────── */
window.wkUnlogSet = async function(sessionExerciseId, setIndex) {
    ...
    if (existingIdx !== -1) {
        const setRec = ex.sets[existingIdx];
        await apexDB.delete('workoutSets', setRec.id);
        ex.sets.splice(existingIdx, 1);
        await window.wkTodayHelpers.persistSession(session);
        window.wkTodayHelpers.emit('setDeleted');
    }
};

window.wkToggleSetLog = async function(sessionExerciseId, setIndex) {
    const session = window.wkState && window.wkState.activeSession;
    if (!session) return;
    const ex = (session.exercises || []).find(e => e.id === sessionExerciseId);
    if (!ex) return;
    ...
    const existingIdx = (ex.sets || []).findIndex(s => s.setIndex === setIndex);
    if (existingIdx !== -1) {
        await window.wkUnlogSet(sessionExerciseId, setIndex);
    } else {
        await window.wkAutoLogSet(sessionExerciseId, setIndex);
    }
};

window.wkAutoLogSet = async function(sessionExerciseId, setIndex, overrideState = {}) {
    const session = window.wkState && window.wkState.activeSession;
    if (!session) return;
    const ex = (session.exercises || []).find(e => e.id === sessionExerciseId);
    if (!ex) return;
    ...
    if (existing) return; // do not auto-log over an existing set
    ...
    const repTarget = ex.repMax != null ? ex.repMax : (ex.repMin != null ? ex.repMin : null);
    const wTarget   = ex.targetWeight != null ? ex.targetWeight : null;

    const state = {
        weight: prevSet && prevSet.weight != null ? prevSet.weight : wTarget,
        reps:   prevSet && prevSet.reps != null ? prevSet.reps : repTarget,
        addedWeight: prevSet && prevSet.addedWeight != null ? prevSet.addedWeight : null,
        durationSec: prevSet && prevSet.durationSec != null ? prevSet.durationSec : null,
        distance: prevSet && prevSet.distance != null ? prevSet.distance : (ex.targetDistance != null ? ex.targetDistance : null),
        distanceUnit: prevSet ? (prevSet.distanceUnit || 'km') : 'km',
        type:   'working',
        notes:  '',
        ...overrideState
    };
    ...
    let prs = null;
    if (tt === 'weight_reps' && record.type !== 'warmup') {
        prs = await window.wkTodayHelpers.checkAndUpdatePR(ex.exerciseId, record);
    }

    // Since this is auto-log, we may be in the runner flow, but we MUST emit to trigger _syncStepsToLogs
    window.wkTodayHelpers.emit('setLogged');
    if (typeof toast === 'function') toast(prs ? '🏆 New personal record!' : 'Set automatically logged');
};
...
window.wkOpenSetModal = function(sessionExerciseId, setIndex) {
    ...
    const state = {
        weight: existing && existing.weight != null ? existing.weight : (prevSet && prevSet.weight != null ? prevSet.weight : wTarget),
        reps:   existing && existing.reps != null ? existing.reps : (prevSet && prevSet.reps != null ? prevSet.reps : repTarget),
        addedWeight: existing && existing.addedWeight != null ? existing.addedWeight : (prevSet && prevSet.addedWeight != null ? prevSet.addedWeight : null),
        durationSec: existing && existing.durationSec != null ? existing.durationSec : (prevSet && prevSet.durationSec != null ? prevSet.durationSec : null),
        distance: existing && existing.distance != null ? existing.distance : (prevSet && prevSet.distance != null ? prevSet.distance : (ex.targetDistance != null ? ex.targetDistance : null)),
    ...
    function fieldStepper(label, key, stepVal, opts = {}) {
        ...
        <div class="wk-today-field">
            ...
            <div class="wk-today-stepper">
                ...
                <input type="number" id="wkSet_${key}" value="${state[key] != null ? state[key] : ''}" min="${min}" step="${stepVal}" />
                <button data-step="${key}" data-delta="${stepVal}" type="button">+</button>
            </div>
        </div>`;
    }
    ...
    function fieldDuration() {
        const m = state.durationSec != null ? Math.floor(state.durationSec / 60) : '';
        const s = state.durationSec != null ? state.durationSec % 60 : '';
        return `
        <div class="wk-today-field">
    ...
    if (tt === 'weight_reps') {
        ...
        <div class="wk-today-field">
            ...
            <div style="display:grid;grid-template-columns:1fr 90px;gap:8px;">
                <input type="number" id="wkSet_distance" class="wk-today-input" min="0" step="0.1" value="${state.distance != null ? state.distance : ''}" style="text-align:center;font-size:18px;font-weight:800;" />
    ...
    overlay.querySelectorAll('input[type=number]').forEach(inp => {
        inp.oninput = () => {
            ...
            state[key] = inp.value === '' ? null : Number(inp.value);
        };
    });
    ...
    async function save() {
        ...
        const durMin = overlay.querySelector('#wkSet_durMin');
        const durSec = overlay.querySelector('#wkSet_durSec');
        if (durMin && durSec) {
            if (durMin.value === '' && durSec.value === '') {
                state.durationSec = null;
            } else {
                state.durationSec = (Number(durMin.value) || 0) * 60 + (Number(durSec.value) || 0);
            }
        }
        const distInp = overlay.querySelector('#wkSet_distance');
        const distUnitSel = overlay.querySelector('#wkSet_distanceUnit');
        if (distInp) state.distance = distInp.value === '' ? null : Number(distInp.value);
